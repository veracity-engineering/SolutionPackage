...
namespace DNVGL.OAuth.Web.TokenCache
{
	public class MsalClientAppBuilder : IClientAppBuilder
	{
		...
		public MsalClientAppBuilder(ITokenCacheProvider tokenCacheProvider = null)
		{
		...
		public IClientApp BuildForClientCredentials()
		{
			...
			if (!string.IsNullOrWhiteSpace(_options.ClientSecret)) builder.WithClientSecret(_options.ClientSecret);
		...
		public IClientApp BuildForUserCredentials(HttpContext httpContext, string codeVerifier = null)
		{
			...
			if (!string.IsNullOrWhiteSpace(codeVerifier)) builder.WithExtraQueryParameters($"code_verifier={codeVerifier}");
