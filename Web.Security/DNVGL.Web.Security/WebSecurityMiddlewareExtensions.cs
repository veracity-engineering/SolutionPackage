...
namespace DNVGL.Web.Security
{
	...
	public static class WebSecurityMiddlewareExtensions
	{
		...
		{
			return builder.UseDefaultSecurityHeaders(async (ctx, next, headerSetter) =>
			{
				headerSetter(ctx);
				await next();
			}, apiPredicate, exceptionPredicate, customizeHeaders);
		}
		...
		/// <param name="customizeHeaders"></param>
		/// <returns>The <see cref="IApplicationBuilder"/>.</returns>
		public static IApplicationBuilder UseDefaultSecurityHeaders(this IApplicationBuilder builder, Func<HttpContext, Func<Task>, Action<HttpContext>, Task> middleware, Func<HttpRequest, bool> apiPredicate = null, Func<HttpRequest, bool> exceptionPredicate = null, Action<IHeaderDictionary> customizeHeaders = null)
		{
			return builder.Use(async (context, next) =>
			{
				await middleware(context, next, ctx =>
				{
